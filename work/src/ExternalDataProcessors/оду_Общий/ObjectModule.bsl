//ВнОбработка ОМ.оду_Общий  
//Основная механика обмена
//TODO:  Переписать модуль, отдельно внешнее соединение, отдельно вызов клиента
...
Процедура РегистрацияДокументаПередЗаписью(Источник, Отказ, РежимЗаписи, РежимПроведения) Экспорт
	...
	Если ПроходитНачальнуюПроверку(Источник, Отказ, Истина) = Ложь Тогда
		Возврат;
	КонецЕсли; 
...
Процедура РегистрацияОбработкаПроведения(Источник, Отказ, РежимПроведения) Экспорт
	...
	Если ПроходитНачальнуюПроверку(Источник, Отказ) = Ложь Тогда
		Возврат;
	КонецЕсли; 
...
Процедура РегистрацияПриЗаписи(Источник, Отказ) Экспорт
	...
	Если ТипЗнч(Источник) = Тип("СправочникОбъект.оду_БазыУчастникиОбмена") ИЛИ
		...
		Если ТипЗнч(Источник) = Тип("СправочникОбъект.оду_БазыУчастникиОбмена") Тогда
	...
	//Уже зареган
	//РезультатРегистрацииОбъектаОбмена = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Источник.ДополнительныеСвойства, "оду_РезультатРегистрацииОбъектаОбмена", Неопределено);
	Если Источник.ДополнительныеСвойства.Свойство("оду_РезультатРегистрацииОбъектаОбмена") Тогда
		Возврат;
	КонецЕсли; 
...
//Регистры сведений 
Процедура оду_ЗарегистрироватьИзменениеРСПередЗаписью(Источник, Отказ, Замещение) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	оду_ОбъектЗарегистрирован = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Источник.ДополнительныеСвойства, "оду_ОбъектЗарегистрирован", Ложь);
	
	Если оду_ОбъектЗарегистрирован = Истина Тогда 
		Источник.ДополнительныеСвойства.Вставить("оду_ОбъектЗарегистрирован", Истина);
		Возврат;
	КонецЕсли;
	...
	Модуль = пр_НастройкиПовтИсп.ИсполняемыйМодуль("оду_Общий", пр_НастройкиПовтИсп.ТекущийПользователь()); 
	Модуль.ЗарегистрироватьИзменениеРСПередЗаписью(Источник, Отказ, Замещение);
	
КонецПроцедуры

&НаСервере
Функция ПроходитНачальнуюПроверку(Источник, Отказ, ОбработчикПередЗаписью = Ложь)
	...
	Если ОбработчикПередЗаписью = Истина Тогда
...
Функция ИмяФайлаОбмена(НастройкиТекущейБД, ИмяКаталогаПриемника)  Экспорт
	...
	Если ЗначениеЗаполнено(ПолныйПутьККаталогу)  Тогда
		...
		ИмяФайлаОбмена = ПолныйПутьККаталогу + "\UOD " + ДатаОбмена;// + " " + пр_Общий.СгенерироватьСлучайныйТекст(3);
...
Функция ИмяКаталогаОбменаУчастникаОбмена(Код, Сервер) Экспорт  
	
	ИмяКаталогаССервера = СтрРазделить(Сервер, ".").ВГраница() = 3;    
	ИмяКаталогаОбмена = ?(ИмяКаталогаССервера = Истина, Код + "_" + Сервер, Код);
	Возврат  ИмяКаталогаОбмена;
	
КонецФункции

Функция ТекстЗапросаПроверкиУО()
	Возврат
		"ВЫБРАТЬ
...
Процедура ПроверкаУзловОбмена()	Экспорт
	...
	Модуль = пр_НастройкиПовтИсп.ИсполняемыйМодуль("оду_Общий", пр_НастройкиПовтИсп.ТекущийПользователь()); 
	РезультатПроверкиУО = Модуль.РезультатПроверкиУО();
	
КонецПроцедуры

Функция РезультатПроверкиУО() Экспорт 
	...
	Запрос = Новый Запрос;
	Запрос.Текст = ТекстЗапросаПроверкиУО();
	...
	Попытка
		...
		Если РезультатЗапроса.Пустой() Тогда
			Возврат РезультатПроверкиИзмененияВерсииУО();
		КонецЕсли; 
		...
		Пока ВДЗ.Следующий() Цикл
			...
			Иначе          
				...
				УзелОбъект.ИмяКаталогаОбмена = ИмяКаталогаОбменаУчастникаОбмена(УзелОбъект.Код, УзелОбъект.Сервер);	
	...
	//TODO:  Добавить регистрацию при изменении настроек УО И отправку в места использования
			...
			ДанныеПользователя = СлужебныйПользовательДляРаботыСОбменом();
			...
			//УзелОбъект.ТекущаяВерсияКонфигурации = СтандартныеПодсистемыСервер.ВерсияБиблиотеки(); 
			УзелОбъект.ТекущаяВерсияКонфигурации = Метаданные.Версия; 
	...
	Исключение
		ЗафиксироватьОшибкуExchange("РезультатПроверкиУО");
	 	Возврат Ложь;
	КонецПопытки;
...
Функция РезультатПроверкиИзмененияВерсииУО()	Экспорт
	...
	Попытка
		...
		Запрос.Текст = 
			"ВЫБРАТЬ
			|	оду_БазыУчастникиОбмена.ТекущаяВерсияКонфигурации КАК ТекущаяВерсияКонфигурации,
			|	оду_БазыУчастникиОбмена.Ссылка КАК Ссылка
			|ИЗ
			|	Справочник.оду_БазыУчастникиОбмена КАК оду_БазыУчастникиОбмена
			|ГДЕ
			...
			|	И оду_БазыУчастникиОбмена.ТекущаяВерсияКонфигурации <> &ТекущаяВерсияКонфигурации";
		
		
		//ВерсияБСП =  СтандартныеПодсистемыСервер.ВерсияБиблиотеки();
		//Запрос.УстановитьПараметр("ВерсияБСП", ВерсияБСП);                
		ТекущаяВерсияКонфигурации = Метаданные.Версия;
		Запрос.УстановитьПараметр("ТекущаяВерсияКонфигурации", ТекущаяВерсияКонфигурации);
		РезультатЗапроса = Запрос.Выполнить();
		...
		Если ВДЗ.Следующий() Тогда
			УзелОбъект = ВДЗ.Ссылка.ПолучитьОбъект();
			//УзелОбъект.ТекущаяВерсияКонфигурации = ВерсияБСП; 
			УзелОбъект.ТекущаяВерсияКонфигурации = ТекущаяВерсияКонфигурации; 
			УзелОбъект.ОбменДанными.Загрузка = Истина;
			УзелОбъект.Записать();
		КонецЕсли; 
		
	Исключение
		ЗафиксироватьОшибкуExchange("РезультатПроверкиИзмененияВерсииУО");
	 	Возврат Ложь;
	КонецПопытки; 
 	 
КонецФункции

Процедура ВывестиЗафиксироватьОшибку(ОтправитьВТелеграм = Истина, Метод = "") Экспорт
	
	пр_Ошибка = ОписаниеОшибки();
	Сообщение = Новый СообщениеПользователю;
	Сообщение.Текст = пр_Ошибка;
	Сообщение.Сообщить(); 
	Если ОтправитьВТелеграм = Истина Тогда
	КонецЕсли; 
	
КонецПроцедуры

Функция СлужебныйПользовательДляРаботыСОбменом() Экспорт
	...
	Если ПользовательИзСправочника = Неопределено Тогда
		ДанныеПользователя = СоздатьСлужебногоПользователяДляРаботыСОбменомДалее(Логин, Пароль);
	Иначе
...
Функция СоздатьСлужебногоПользователяДляРаботыСОбменом() Экспорт
	Возврат СлужебныйПользовательДляРаботыСОбменом();
КонецФункции 

//TODO:  Доработка возврата пароля
Функция СоздатьСлужебногоПользователяДляРаботыСОбменомДалее(Логин = "uExchange", Пароль = "exc-a9a7") Экспорт
...
Функция РегистрироватьИсточникОбмена(Источник) Экспорт
	...
	РегистрироватьОбъектОбмена = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Источник.ДополнительныеСвойства, "оду_РегистрироватьОбъектОбмена", Ложь);
	РегистрироватьБезПроверок = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Источник.ДополнительныеСвойства, "оду_РегистрироватьБезПроверок", Ложь);
	
	Если РегистрироватьОбъектОбмена = Истина ИЛИ РегистрироватьБезПроверок = Истина Тогда
		Возврат Истина;
	КонецЕсли;
...
Функция ПараметрыКонвертацииОбъекта()  Экспорт
	...
	ПараметрыКонвертации.Вставить("ИмяКаталогаОбмена", "");
	...
	ПараметрыКонвертации.Вставить("ПланОбмена", Неопределено);
...
Функция ЗафиксироватьОшибкуExchange(Метод, ОтправлятьВТелеграмм = Истина, ПоказатьОшибку = Истина, Объект1С = "", ОшибкаДляТелеграмм = "", БазаУО = "", Версия = "", Данные = Неопределено, ДопИнформация = "")  Экспорт
...
Функция ТекстЗапросаОбновленияСостояниямиОбмена(Источник = Ложь, ТолькоОбновитьСостояния = Ложь)
	...
	ТекстЗапроса = 	
		...
		|	ВТ_ДАнные.ДополнительныеСвойстваОбъекта КАК ДополнительныеСвойстваОбъекта,
	...
	Если  ТолькоОбновитьСостояния = Ложь Тогда
		...
		|	СостояниеВыгрузки.ДополнительныеСвойстваОбъекта,
...
Функция РезультатОбновленияСостоянийОбмена(ТелоЗапроса, ИмяСвойства = "data", ТолькоОбновитьСостояния = Ложь)  Экспорт
	...
	Попытка
		...
		Пока ВДЗ.Следующий() Цикл
			...
			Если ВДЗ.РежимОбмена = оду_ОбщийПовтИсп.РежимВыгрузка() Тогда
				...
				Иначе	
					...
					//ИначеЕсли ВДЗ.СостояниеНеМенялось = Ложь Тогда //Проверка изменения состояний
		...
		//TODO:  Подумать как запустить с отсрочкой
		...
		Если ЕстьДанныеДляЗагрузки = Истина ИЛИ ТолькоОбновитьСостояния = Истина Тогда
			
			ИмяПроцедуры = "оду_Общий.ВыполнитьОтложенноеДействиеОбмена";
			МассивПараметров = Новый Массив;
			МассивПараметров.Добавить(ЕстьДанныеДляЗагрузки);
			МассивПараметров.Добавить(ТолькоОбновитьСостояния);
			
			пр_Общий.ЗапуститьФоновоеВыполнение(ИмяПроцедуры, МассивПараметров, , "оду_ВыполнитьОтложенноеДействиеОбмена"); 
...
Функция РезультатExchangeStatusComplet(ТелоЗапроса)
	...
	МассивДанныхЗапросаСсылки = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Данные, "МассивДанныхЗапросаСсылки", Неопределено);
	Если МассивДанныхЗапросаСсылки <> Неопределено Тогда
...
Функция ВыполнитьОтложенноеДействиеОбмена(ЕстьДанныеДляЗагрузки = Ложь, ТолькоОбновитьСостояния = Ложь, КолСекунд = 5)  Экспорт   
	
	пр_КлиентСервер.Подождать(КолСекунд);	
...
Функция ТекстЗапросаПолученияПлановОбменаРС()
	Возврат
	
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	оду_БазыУчастникиОбменаСостав.Ссылка КАК Ссылка,
		...
		|	оду_БазыУчастникиОбменаСостав.НомерСтроки КАК НомерСтроки,
		|	оду_БазыУчастникиОбменаСостав.Объект КАК Объект,
		|	оду_БазыУчастникиОбменаСостав.ПравилоРегистрации КАК ПравилоРегистрации,
		|	оду_БазыУчастникиОбменаСостав.НастройкаКонвертацииОбмена КАК НастройкаКонвертацииОбмена
		|ПОМЕСТИТЬ ВТ_ОтборНачальныеОтборы
		|ИЗ
		|	Справочник.оду_БазыУчастникиОбмена.Состав КАК оду_БазыУчастникиОбменаСостав
		|ГДЕ
		|	оду_БазыУчастникиОбменаСостав.Ссылка.Используется = ИСТИНА
		...
		|			ИНАЧЕ ИСТИНА
		|		КОНЕЦ
		|	И оду_БазыУчастникиОбменаСостав.ТипXMLОбъектаИсточника = &ТипXML
		|	И оду_БазыУчастникиОбменаСостав.Регистрировать = ИСТИНА
		|	И оду_БазыУчастникиОбменаСостав.Ссылка.ПодчиненныйУчастникОбмена = ЛОЖЬ
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	оду_БазыУчастникиОбменаСостав.Ссылка,
		|	оду_БазыУчастникиОбменаСостав.ТипXMLОбъектаИсточника,
		|	оду_БазыУчастникиОбменаСостав.ТипXMLОбъектаПриемника,
		|	оду_БазыУчастникиОбменаСостав.НомерСтроки,
		|	оду_БазыУчастникиОбменаСостав.Объект,
		|	оду_БазыУчастникиОбменаСостав.ПравилоРегистрации,
		|	оду_БазыУчастникиОбменаСостав.НастройкаКонвертацииОбмена
		|ИЗ
		|	Справочник.оду_БазыУчастникиОбмена.Состав КАК оду_БазыУчастникиОбменаСостав
		|ГДЕ
		|	оду_БазыУчастникиОбменаСостав.Ссылка.Используется = ИСТИНА
		...
		|	И оду_БазыУчастникиОбменаСостав.Ссылка.РаспределятьПодчиненнымУО = ИСТИНА
		|	И оду_БазыУчастникиОбменаСостав.ТипXMLОбъектаИсточника = &ТипXML
		|	И оду_БазыУчастникиОбменаСостав.Регистрировать = ИСТИНА
		|	И ВЫБОР
		|			КОГДА оду_БазыУчастникиОбменаСостав.Ссылка.ЭтоРабочаяБД = ИСТИНА
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_ОтборНачальныеОтборы.Ссылка КАК БазаПриемник,
		|	ВЫБОР
		|		КОГДА ЕСТЬNULL(ВТ_ОтборНачальныеОтборы.ПравилоРегистрации, ЗНАЧЕНИЕ(Справочник.оду_ПравилаРегистрацииОбмена.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Справочник.оду_ПравилаРегистрацииОбмена.ПустаяСсылка)
		|				И ВТ_ОтборНачальныеОтборы.ПравилоРегистрации.ПометкаУдаления = ЛОЖЬ
		|				И ВТ_ОтборНачальныеОтборы.ПравилоРегистрации.Используется = ИСТИНА
		|			ТОГДА ВТ_ОтборНачальныеОтборы.ПравилоРегистрации
		|		ИНАЧЕ ВТ_ОтборНачальныеОтборы.Ссылка.ПравилоРегистрации
		|	КОНЕЦ КАК НастройкаПравилаРегистрации,
		|	ВЫБОР
		|		КОГДА ЕСТЬNULL(ВТ_ОтборНачальныеОтборы.НастройкаКонвертацииОбмена, ЗНАЧЕНИЕ(Справочник.оду_НастройкаКонвертацииДанных.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Справочник.оду_НастройкаКонвертацииДанных.ПустаяСсылка)
		|				И ВТ_ОтборНачальныеОтборы.НастройкаКонвертацииОбмена.ПометкаУдаления = ЛОЖЬ
		|			ТОГДА ВТ_ОтборНачальныеОтборы.НастройкаКонвертацииОбмена
		|		ИНАЧЕ ВТ_ОтборНачальныеОтборы.Ссылка.НастройкаКонвертацииОбмена
		|	КОНЕЦ КАК НастройкаКонвертацииОбмена,
		|	ВТ_ОтборНачальныеОтборы.ТипXMLОбъектаИсточника КАК ТипXMLОбъектаИсточника,
		|	ВТ_ОтборНачальныеОтборы.ТипXMLОбъектаПриемника КАК ТипXMLОбъектаПриемника,
		|	ВТ_ОтборНачальныеОтборы.Ссылка.ИДБазы КАК ИДБазыПриемник,
		|	ВТ_ОтборНачальныеОтборы.Ссылка.ИДКонфигурации КАК ИДКонфигурации,
		|	ВТ_ОтборНачальныеОтборы.Ссылка.ЭтоРабочаяБД КАК ЭтоРабочаяБД,
		|	ВТ_ОтборНачальныеОтборы.НомерСтроки КАК ПриоритетОбъекта,
		|	ВТ_ОтборНачальныеОтборы.Объект.Родитель.Наименование КАК МетаданныеНаименование,
		|	ВТ_ОтборНачальныеОтборы.Ссылка.ПриоритетОбмена КАК ПриоритетОбмена,
		|	ВТ_ОтборНачальныеОтборы.Ссылка.РежимОтладки КАК РежимОтладки
		|ПОМЕСТИТЬ ВТ_Настройки
		|ИЗ
		|	ВТ_ОтборНачальныеОтборы КАК ВТ_ОтборНачальныеОтборы
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ВТ_Настройки.БазаПриемник.ПланОбмена КАК ПланОбмена
		|ИЗ
		|	ВТ_Настройки КАК ВТ_Настройки
		|ГДЕ
		|	ЕСТЬNULL(ВТ_Настройки.БазаПриемник.ПланОбмена.ПометкаУдаления, ИСТИНА) = ЛОЖЬ 
		|	И ЕСТЬNULL(ВТ_Настройки.БазаПриемник.ПланОбмена.ЭтотУзел, ИСТИНА) = ЛОЖЬ
		|
		|УПОРЯДОЧИТЬ ПО
		|	ПриоритетОбмена";
...
Процедура ЗарегистрироватьИзменениеРСПередЗаписью(Источник, Отказ, Замещение)  Экспорт 
	...
	Запрос = Новый Запрос;
	Запрос.Текст = ТекстЗапросаПолученияПлановОбменаРС();  
	
	ИмяТипаXML = пр_Общий.ИмяТипаXMLПоСсылке(Источник);  
	
	Запрос.УстановитьПараметр("ТипXML", ИмяТипаXML);
	...
	Пока ВДЗ.Следующий() Цикл
		...
		РазрешенаРегистрацияОбъектаОбменаПоОтбору = Модуль.РазрешенаРегистрацияОбъектаОбменаПоОтбору(Источник, ВДЗ); 
		Если РазрешенаРегистрацияОбъектаОбменаПоОтбору = Истина Тогда
			Источник.ОбменДанными.Получатели.Добавить(ВДЗ.ПланОбмена);
			Источник.ДополнительныеСвойства.Вставить("оду_ОбъектЗарегистрирован", Истина);
		КонецЕсли; 	
	КонецЦикла;
	...
	//ОбъектОбмена = Источник;  
	//МассивОбъектовОбмена = Новый Массив;
	//МассивОбъектовОбмена.Добавить(ОбъектОбмена);					  
	//ПараметрыРегистрации = ПараметрыРегистрацииУчастниковОбменаПоТипуОбъекта(МассивОбъектовОбмена);
	//
	...
	//КонецЕсли;
	//
	////ЗаполнитьСтруктурыДопДанными(МассивУчастниковОбмена, ОбъектОбмена, ПараметрыРегистрации.ТекущийПользователь);
	//
	...
	//	//КонецЕсли; 
	//	
	//	Модуль = пр_НастройкиПовтИсп.ИсполняемыйМодуль("оду_ОбменДанными", ПараметрыРегистрации.ТекущийПользователь); 
	...
	//	//Регистрация объекта	
	//	Если РазрешенаРегистрацияОбъектаОбменаПоОтбору = Истина Тогда
	//		//Источник.ОбменДанными.Получатели.Добавить(ВДЗ.Ссылка);	
	//		//РезультатРегистрацииОбъектаОбмена = РезультатРегистрацииОбъектаОбмена(СтруктураНастроекОбмена); 
	//	КонецЕсли; 
	//	
	//	//Источник.ДополнительныеСвойства.Вставить("оду_РезультатРегистрацииОбъектаОбмена", РезультатРегистрацииОбъектаОбмена);
	//	//
...
Процедура ЗарегистрироватьИзменениеОбъектаОбмена(Источник, Отказ, РежимЗаписи = Неопределено, РежимПроведения = Неопределено, ПриоритетДопПризнака = 0) Экспорт
	...
	ЗаполнитьСтруктурыДопДанными(МассивУчастниковОбмена, ОбъектОбмена, ПараметрыРегистрации.ТекущийПользователь, , РежимЗаписи, РежимПроведения);
	РегистрироватьБезПроверок = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Источник.ДополнительныеСвойства, "оду_РегистрироватьБезПроверок", Ложь);
	...
	Для каждого СтруктураНастроекОбмена Из МассивУчастниковОбмена Цикл
		...
		ЗаполнитьДополнительныеСвойстваОбъектаПоИсточнику(Источник, СтруктураНастроекОбмена);
		
		РезультатРегистрацииОбъектаОбмена = Ложь;
		
		Если РегистрироватьБезПроверок = Истина Тогда
			//РазрешенаРегистрацияОбъектаОбменаПоОтбору = Истина;
			//Регистрация объекта	
			РезультатРегистрацииОбъектаОбмена = РезультатРегистрацииОбъектаОбмена(СтруктураНастроекОбмена, , РегистрироватьБезПроверок); 
		Иначе
			
			//1. тк может поменяться СтруктураНастроекОбмена, с распределения на конктретные УО по отбору	
			Модуль = пр_НастройкиПовтИсп.ИсполняемыйМодуль("оду_ОбменДанными", ПараметрыРегистрации.ТекущийПользователь); 
			...
			Если РазрешенаРегистрацияОбъектаОбменаПоОтбору = Истина Тогда
				...
				Если ТипЗнч(СтруктураНастроекОбмена) = Тип("Массив") Тогда
					Для каждого ТекСтруктураНастроекОбмена Из СтруктураНастроекОбмена Цикл
						РезультатРегистрацииОбъектаОбмена = РезультатРегистрацииОбъектаОбменаПоСтруктуреНастроекОбмена(ТекСтруктураНастроекОбмена, Источник, ТекСтруктураНастроекОбмена);	
					КонецЦикла;
				Иначе
					РезультатРегистрацииОбъектаОбмена = РезультатРегистрацииОбъектаОбменаПоСтруктуреНастроекОбмена(СтруктураНастроекОбмена, Источник, РегистрироватьБезПроверок);	
				КонецЕсли;
			...
			//КонецЕсли;         
			//
			////Доп проверка на отбор регистрации   
			//ЭтоОбмен = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Источник.ДополнительныеСвойства, "ЭтоОбмен", Ложь);
			//
			//Если ЭтоОбмен = Ложь Тогда
			//	Если ОбъектОбменаПроходитПроверкуДляРегистрации(СтруктураНастроекОбмена, Источник) = Ложь Тогда
			//		Продолжить;
			//	КонецЕсли; 
			//КонецЕсли;
		...
		//TODO:  подумать, нужно ли запрещать изменять объект, если не удалось зарегать
		// Возможно реализовать кастомизацию параметра     
		Если ТипЗнч(СтруктураНастроекОбмена) = Тип("Структура") Тогда
			Если СтруктураНастроекОбмена.Отказ = Истина ИЛИ РезультатРегистрацииОбъектаОбмена = Ложь Тогда
...
Функция РезультатРегистрацииОбъектаОбменаПоСтруктуреНастроекОбмена(СтруктураНастроекОбмена, Источник, РегистрироватьБезПроверок)
...
Процедура ЗаполнитьДополнительныеСвойстваОбъектаПоИсточнику(Источник, СтруктураНастроекОбмена)
	
	Попытка
		
		Если Источник.ДополнительныеСвойства.Количество() = 0 Тогда
			Возврат;	
		КонецЕсли;
		
			
		оду_ДополнительныеСвойстваРегистрации = Неопределено;
		Источник.ДополнительныеСвойства.Свойство("оду_ДополнительныеСвойстваРегистрации", оду_ДополнительныеСвойстваРегистрации);
		//оду_ДополнительныеСвойстваРегистрации = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Источник.ДополнительныеСвойства,"оду_ДополнительныеСвойстваРегистрации", Неопределено);
		
		Если ЗначениеЗаполнено(оду_ДополнительныеСвойстваРегистрации) Тогда
			СтруктураНастроекОбмена.Вставить("ДополнительныеСвойстваОбъекта", оду_ДополнительныеСвойстваРегистрации);
		КонецЕсли;
		
		//ДополнительныеСвойстваОбъекта = Новый Структура();
		//
		//Для каждого КлючЗначение Из Источник.ДополнительныеСвойства Цикл
		//	ИмяДопСвойства = КлючЗначение.Ключ; 
		//	Если СтрНайти(ИмяДопСвойства, "оду_") Тогда
		//		ДополнительныеСвойстваОбъекта.Вставить(ИмяДопСвойства, КлючЗначение.Значение);
		//	КонецЕсли;
		//КонецЦикла;
		//
	...
	Исключение
		ЗафиксироватьОшибкуExchange("ЗаполнитьДополнительныеСвойстваОбъектаПоИсточнику",,,,ОписаниеОшибки());
	КонецПопытки;
...
Функция ПараметрыРегистрацииУчастниковОбменаПоТипуОбъекта(МассивОбъектовОбмена, ТекущийПользователь = Неопределено, ЭтоСамолет = Ложь, УОД = Неопределено) Экспорт
	
	Если ТекущийПользователь = Неопределено Тогда
		ТекущийПользователь = пр_НастройкиПовтИсп.ТекущийПользователь();
	КонецЕсли;
	
	ИмяТипаXML = пр_Общий.ИмяТипаXMLПоСсылке(МассивОбъектовОбмена[0]); 
	НастройкиТекущейБД = оду_ОбщийПовтИсп.НастройкиТекущейБД();
    
    Если НастройкиТекущейБД = Неопределено Тогда  
        Сообщение = Новый СообщениеПользователю;
        Сообщение.Текст = "Не удалось сформировать настройки текущей базы";
        Сообщение.Сообщить();
		Возврат Неопределено;
	КонецЕсли; 
	
	СтруктураДанных = СтруктураДанныхОбъектовОбмена(ИмяТипаXML, МассивОбъектовОбмена);
	Если СтруктураДанных = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли; 
	...
	//МассивУчастниковОбмена = Модуль.МассивУчастниковОбменаПоТипуОбъектаИОрганизаций(ИмяТипаXML, НастройкиТекущейБД, ЭтоСамолет, МассивОрганизаций);
	
	пр_Модуль = пр_Общий.ИсполняемыйМодуль("оду_Общий", ТекущийПользователь); 
	МассивУчастниковОбмена = пр_Модуль.МассивУчастниковОбменаПоТипуОбъектаИОрганизаций(ИмяТипаXML, НастройкиТекущейБД, ЭтоСамолет, МассивОрганизаций, УОД);
	
	Если МассивУчастниковОбмена = Неопределено И ЭтоСамолет = Истина Тогда
		Сообщение = Новый СообщениеПользователю;
...
#Область ПолученияМассиваУчастниковОбменаПоТипуОбъектаИОрганизаций 

Функция ТекстЗапросаПолученияМассиваУчастниковОбменаПоТипуОбъектаИОрганизаций(ЭтоСамолет = Ложь, МассивОрганизаций = Неопределено, УОД = Неопределено)
	
	ТекстЗапроса = 
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	оду_БазыУчастникиОбмена.Ссылка КАК Ссылка
		|ПОМЕСТИТЬ ВТ_ОтборПоОрганизациям
		|ИЗ
		|	Справочник.оду_БазыУчастникиОбмена КАК оду_БазыУчастникиОбмена
		...
		|				И оду_БазыУчастникиОбменаОрганизации.Организация ССЫЛКА Справочник.Организации)
		|ГДЕ
		|	оду_БазыУчастникиОбмена.Ссылка.Используется = ИСТИНА
		|	И оду_БазыУчастникиОбмена.Ссылка.ПометкаУдаления = ЛОЖЬ    
		|	И ВЫБОР
		|			КОГДА оду_БазыУчастникиОбмена.РаспределятьПодчиненнымУО
		|				ТОГДА ИСТИНА
		|			ИНАЧЕ оду_БазыУчастникиОбмена.ТекущаяБД = ЛОЖЬ
		|		КОНЕЦ
		...
		|	оду_БазыУчастникиОбменаСостав.Ссылка.ТекущаяБД КАК ТекущаяБД,
		...
		|	оду_БазыУчастникиОбменаСостав.Ссылка.ПодчиненныйУчастникОбмена КАК ПодчиненныйУчастникОбмена,
		|	оду_БазыУчастникиОбменаСостав.Ссылка.РаспределятьПодчиненнымУО КАК РаспределятьПодчиненнымУО
		|ПОМЕСТИТЬ ВТ_Настройки
		|ИЗ
		|	ВТ_ОтборПоОрганизациям КАК ВТ_ОтборПоОрганизациям
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.оду_БазыУчастникиОбмена.Состав КАК оду_БазыУчастникиОбменаСостав
		|		ПО ВТ_ОтборПоОрганизациям.Ссылка = оду_БазыУчастникиОбменаСостав.Ссылка
		|ГДЕ
		...
		|	ВТ_Настройки.ИДКонфигурации КАК ИДКонфигурации,
		|	ВТ_Настройки.ЭтоРабочаяБД КАК ЭтоРабочаяБД,
		|	ВТ_Настройки.ТекущаяБД КАК ТекущаяБД,
		|	&ТекущаяРабочаяБД КАК ТекущаяБДРабочая,
		|	ВТ_Настройки.ПриоритетОбъекта КАК ПриоритетОбъекта,
		|	ВТ_Настройки.МетаданныеНаименование КАК МетаданныеНаименование,
		|	ВТ_Настройки.ПриоритетОбмена КАК ПриоритетОбмена,
		...
		|	&КаталогОбмена КАК КаталогОбмена,
		|	ВТ_Настройки.БазаПриемник.ПланОбмена КАК ПланОбмена,
		|	ЕСТЬNULL(ВТ_Настройки.БазаПриемник.ПланОбмена.ПометкаУдаления, ИСТИНА) = ЛОЖЬ КАК ИспользуетПланОбмена,
		|	ВТ_Настройки.ПодчиненныйУчастникОбмена КАК ПодчиненныйУчастникОбмена,
		|	ВТ_Настройки.РаспределятьПодчиненнымУО КАК РаспределятьПодчиненнымУО,
	...
	Отбор = "";
	Если УОД  <> Неопределено Тогда
		Отбор = "
			|	И оду_БазыУчастникиОбмена.Ссылка = &УОД 
		    |";
	КонецЕсли;
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "#ОтборУОД", Отбор);
	...
	Если МассивОрганизаций <> Неопределено Тогда
		Отбор = "
			|	И ВЫБОР
			|			КОГДА оду_БазыУчастникиОбменаОрганизации.Организация ЕСТЬ NULL
			|				ТОГДА ИСТИНА
			|			ИНАЧЕ оду_БазыУчастникиОбменаОрганизации.Организация В (&МассивОрганизаций)
			|		КОНЕЦ
		    |";
	КонецЕсли;
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "#ОтборМассивОрганизаций", Отбор);
...
//TODO:  Для организаций отдельно на оду_Общий
...
Функция МассивУчастниковОбменаПоТипуОбъектаИОрганизаций(ТипXML, НастройкиТекущейБД, ЭтоСамолет = Ложь, МассивОрганизаций = Неопределено, УОД = Неопределено) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст = ТекстЗапросаПолученияМассиваУчастниковОбменаПоТипуОбъектаИОрганизаций(ЭтоСамолет, МассивОрганизаций, УОД);
	Запрос.УстановитьПараметр("МассивОрганизаций", МассивОрганизаций);
	Запрос.УстановитьПараметр("УОД", УОД);
	...
	МассивУчастниковОбмена = пр_Общий.МассивСтруктурПоЗапросу(РЗ);
	
	Для каждого СтруктураНастроек Из МассивУчастниковОбмена Цикл
		
		//СтруктураНастроек = ЗначениеИзСтрокиВнутр(ЗначениеВСтрокуВнутр(СтруктураНастроекН));
		
		ВыборкаОрганизаций = СтруктураНастроек.select.Выбрать();
		select = Новый Массив;
			   
		Пока ВыборкаОрганизаций.Следующий() Цикл
			Если ТипЗнч(ВыборкаОрганизаций.Организация) = Тип("СправочникСсылка.Организации") Тогда
				select.Добавить(ВыборкаОрганизаций.Организация);	
			КонецЕсли; 
		КонецЦикла;
...
//TODO:  Объеденить Функция ПроходитПровекуНаОграничениеОрганизаций(СтруктураНастроекОбмена, Источник)
Функция СтруктураДанныхОбъектовОбмена(ИмяТипаXML, МассивОбъектовОбмена)
	
	Попытка
		
		СтруктураДанных = Новый Структура("МассивОрганизаций, СоответствиеОбъектовОбменаОрганизации", Неопределено, МассивОбъектовОбмена); 
		
		Если НЕ (СтрНайти(ИмяТипаXML, "DocumentRef") <> 0 ИЛИ ИмяТипаXML = "CatalogRef.Организации") Тогда
		...
		ТекстЗапроса = 
			"ВЫБРАТЬ
			|	ОбъектОбмена.#ИмяРеквизита КАК Организация,
			|	ОбъектОбмена.Ссылка КАК Ссылка
			|ИЗ
			|	#ОбъектОбмена КАК ОбъектОбмена
			|ГДЕ
			|	ОбъектОбмена.Ссылка В(&МассивОбъектовОбмена)
			|ИТОГИ ПО
			|	Организация
			|";
		...
		Если ИмяТипаXML = "CatalogRef.Организации" Тогда
			ИмяРеквизита = "Ссылка";
		КонецЕсли; 
		
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "#ИмяРеквизита", ИмяРеквизита); 
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "#ОбъектОбмена", СтрЗаменить(ИмяТипаXML, "Ref", "")); 
		...
		СтруктураДанных.Вставить("МассивОрганизаций", МассивОрганизаций);
		СтруктураДанных.Вставить("СоответствиеОбъектовОбменаОрганизации", СоответствиеОбъектовОбменаОрганизации); 
...
Процедура ПроверкаОбъектовОбменаНаПрохождениеРегистрации(МассивОбъектовОбмена, ТЗ_УчастникиОбмена, СоотОбъектов, ИДОбъекты, СтруктураНастроекОбмена)
	...
	Для каждого ОбъектОбмена Из МассивОбъектовОбмена Цикл
		...
		//Проверка на ограничения    
		Если ПроходитПровекуНаОграничениеОрганизаций(СтруктураНастроекОбмена, ОбъектОбмена) = Ложь Тогда
			Продолжить;
		КонецЕсли;         
		
		//Доп проверка на отбор регистрации 
		Если ОбъектОбменаПроходитПроверкуДляРегистрации(СтруктураНастроекОбмена, ОбъектОбмена) = Ложь Тогда
			Продолжить;
		КонецЕсли; 
			
		//Модуль = пр_НастройкиПовтИсп.ИсполняемыйМодуль("оду_ОбменДанными", ТекущийПользователь); 
		//РазрешенаРегистрацияОбъектаОбменаПоОтбору = Модуль.РазрешенаРегистрацияОбъектаОбменаПоОтбору(ОбъектОбмена, СтруктураНастроекОбмена); 
		//
		////Регистрация объекта	
		//Если РазрешенаРегистрацияОбъектаОбменаПоОтбору = Истина Тогда
			РезультатРегистрацииОбъектаОбмена = РезультатРегистрацииОбъектаОбмена(СтруктураНастроекОбмена); 
		...
		Если РезультатРегистрацииОбъектаОбмена = Истина Тогда
...
Функция РезультатРегистрацииОбъектовОбменаСамолет(СтруктураНастроекОбменаНач, ПриоритетОбмена = 10, ПриоритетДопПризнака = 0, ДатаРегистрации = Неопределено)   Экспорт
	...
	СоответствиеОбъектовОбменаОрганизации = СтруктураНастроекОбменаНач.СоответствиеОбъектовОбменаОрганизации;
	
	Если СоответствиеОбъектовОбменаОрганизации.Количество() = 0 Тогда
		Возврат Ложь;
	КонецЕсли;   
	...
	Для каждого СтруктураНастроекОбмена Из МассивУчастниковОбмена Цикл
		...
		//Перебор Организаций участвующих в обмене
		//Без ограничение на Организации
		Если ТипЗнч(СоответствиеОбъектовОбменаОрганизации) = Тип("Массив") Тогда
			ПроверкаОбъектовОбменаНаПрохождениеРегистрации(СоответствиеОбъектовОбменаОрганизации, ТЗ_УчастникиОбмена, СоотОбъектов, ИДОбъекты, СтруктураНастроекОбмена);
		Иначе	
			
			СписокОграниченийПоОрганизации = СтруктураНастроекОбмена.select;
			
			Если СписокОграниченийПоОрганизации.Количество() = 0 Тогда
				
				Для каждого МассивОбъектовОбмена Из СоответствиеОбъектовОбменаОрганизации Цикл
					ПроверкаОбъектовОбменаНаПрохождениеРегистрации(МассивОбъектовОбмена.Значение, ТЗ_УчастникиОбмена, СоотОбъектов, ИДОбъекты, СтруктураНастроекОбмена);
				КонецЦикла; 	
				
			Иначе
				
				Для каждого Организация Из СписокОграниченийПоОрганизации Цикл
					МассивОбъектовОбмена = СоответствиеОбъектовОбменаОрганизации.Получить(Организация);
					ПроверкаОбъектовОбменаНаПрохождениеРегистрации(МассивОбъектовОбмена, ТЗ_УчастникиОбмена, СоотОбъектов, ИДОбъекты, СтруктураНастроекОбмена);
				КонецЦикла; 
...
Функция СтруктураНастроекОбмена()  Экспорт
...
Функция РезультатРегистрацииОбъектаОбмена(СтруктураНастроекОбмена, Удалить = Ложь, РегистрироватьБезПроверок = Ложь)	Экспорт
	...
	Попытка
		...
		Если РегистрироватьБезПроверок = Истина Тогда
			ЗаполнитьЗаписьРегистрацииИзСтруктурыНастроекОбмена(Запись, СтруктураНастроекОбмена);	
		Иначе
			
			ЗаполнитьЗначенияСвойств(Запись, СтруктураНастроекОбмена, "ИДОбъекта, ИДБазыПриемник"); 
			Запись.Прочитать();
			Если Запись.Выбран() Тогда
				//TODO:  Подумать, если раньше была запись с доп свойствами, потом до отправки еще изменения
				// что делать с доп параметрами, перезаписывать, добавалять, оставлять
				ИменаПолейЗаполнения = "ДатаРегистрации, Пользователь";//, ЭтоУдалениеОбъекта";
				ЗаполнитьЗаписьРегистрацииИзСтруктурыНастроекОбмена(Запись, СтруктураНастроекОбмена, ИменаПолейЗаполнения);//"ДополнительныеСвойстваОбъекта");	
			Иначе
				ЗаполнитьЗаписьРегистрацииИзСтруктурыНастроекОбмена(Запись, СтруктураНастроекОбмена);	
			КонецЕсли; 
	...
	Исключение
		пр_Ошибка = ОписаниеОшибки();
		//пр_Результат = пр_Общий.РезультатЗаписиВЛог("", Объект1С, пр_Ошибка, ТипСсылки, Ссылка, ВходящиеДанные, Пользователь, ПериодЗаписи);
		ОтправитьСообщениеВТелеграмExchange("оду_Общий.РезультатРегистрацииОбъектаОбмена", , пр_Ошибка, СтруктураНастроекОбмена.БазаПриемник, , , СтруктураНастроекОбмена);	
		Результат = Ложь;
	КонецПопытки;	
...
Процедура ЗаполнитьЗаписьРегистрацииИзСтруктурыНастроекОбмена(Запись, СтруктураНастроекОбмена, ИменаПолейЗаполнения = "", ИменаПолейИсключений = "")
	
	Если ИменаПолейЗаполнения = "" Тогда
		ЗаполнитьЗначенияСвойств(Запись, СтруктураНастроекОбмена); 
		
	Иначе 
		ЗаполнитьЗначенияСвойств(Запись, СтруктураНастроекОбмена, ИменаПолейЗаполнения, ИменаПолейИсключений); 
	КонецЕсли; 
	
	//TODO:  подумать
	Если ИменаПолейЗаполнения = "" Тогда
		Если СтруктураНастроекОбмена.Свойство("ДополнительныеСвойстваОбъекта") Тогда
			Если ТипЗнч(СтруктураНастроекОбмена.ДополнительныеСвойстваОбъекта) = Тип("Структура") Тогда
				Запись.ДополнительныеСвойстваОбъекта = ЗначениеВСтрокуВнутр(СтруктураНастроекОбмена.ДополнительныеСвойстваОбъекта);
			ИначеЕсли ТипЗнч(СтруктураНастроекОбмена.ДополнительныеСвойстваОбъекта) = Тип("Строка") Тогда
				Запись.ДополнительныеСвойстваОбъекта = СтруктураНастроекОбмена.ДополнительныеСвойстваОбъекта;
			Иначе
...
#Область  РаспределенныеУО

Функция ТекстЗапросаРаспределенияДанныхОбменаПоПодчиненнымУО()
	Возврат 
	
		"ВЫБРАТЬ
		|	оду_СостояниеОбъектовОбмена.ИДОбъекта КАК ИДОбъекта,
		|	оду_СостояниеОбъектовОбмена.ИДБазыПриемник КАК ИДБазыИсточник,
		|	оду_БазыУчастникиОбмена.ИДБазы КАК ИДБазыПриемник,
		|	&ОтправкаДанных КАК РежимОбмена,
		|	&СостояниеВыгружен КАК СостояниеОбмена,
		|	оду_СостояниеОбъектовОбмена.ТипXMLОбъектаПриемника КАК ТипXMLОбъектаИсточника,
		...
		|	1 КАК НомерПопытки,
		|	"""" КАК ОписаниеОшибки,
		|	ЛОЖЬ КАК СменаИсточникаПриеменикаПриРазрешенииКоллизий,
		...
		|	И оду_БазыУчастникиОбмена.ИДБазы <> оду_СостояниеОбъектовОбмена.ИДБазыИсточник
		|	И оду_БазыУчастникиОбмена.ИДБазы <> оду_СостояниеОбъектовОбмена.ИДБазыПриемник
		|	И оду_БазыУчастникиОбмена.ПометкаУдаления = ЛОЖЬ
		|	И оду_БазыУчастникиОбмена.Используется = ИСТИНА
		|ИТОГИ ПО
		|	ИмяФайлаОбмена,
...
Функция РезультатРаспределенияДанныхОбменаПоПодчиненнымУО(НастройкиТекущейБД, ИмяФайлаОбмена, ПредставлениеИсточника)
	
	Попытка
	
	Если НастройкиТекущейБД.РаспределятьПодчиненнымУО = Ложь Тогда
		Возврат Истина;	
	КонецЕсли; 
	
	Запрос = Новый Запрос;
	Запрос.Текст = ТекстЗапросаРаспределенияДанныхОбменаПоПодчиненнымУО();
	...
	Запрос.УстановитьПараметр("ОтправкаДанных", оду_ОбщийПовтИсп.РежимВыгрузка());
	Запрос.УстановитьПараметр("СостояниеВыгружен", оду_ОбщийПовтИсп.СостояниеВыгружен());
	...
	Возврат РезультатРаспределенияДанныхОбменаПоПодчиненнымУОИзРезультатаЗапроса(РезультатЗапроса, НастройкиТекущейБД, ПредставлениеИсточника);
	
	Исключение
		пр_Ошибка = ОписаниеОшибки();
		ОтправитьСообщениеВТелеграмExchange("РезультатРаспределенияДанныхОбменаПоПодчиненнымУО", , пр_Ошибка, ПредставлениеИсточника);	
	 	Возврат Ложь;
	КонецПопытки;
...
Функция РезультатРаспределенияДанныхОбменаПоПодчиненнымУОИзРезультатаЗапроса(РезультатЗапроса, НастройкиТекущейБД, ПредставлениеИсточника, МассивЗагруженныхФайловОбмена = Неопределено) 	Экспорт
	...
	Попытка
		...
		Пока ВыборкаИмяФайлаОбмена.Следующий() Цикл
			...
			МассивЗагруженныхФайловОбмена.Добавить(ВыборкаИмяФайлаОбмена.ИмяФайлаОбмена);
			...
			Пока ВыборкаИмяКаталогаОбмена.Следующий() Цикл
				...
				КаталогОбмена = КаталогФайлаОбмена(НастройкиТекущейБД, ВыборкаИмяКаталогаОбмена.ИмяКаталогаПриемника);
...
Функция ИмяФайлаПриемникаДляПодчиненноУО(ИмяТекущегоФайлаОбмена, КаталогаОбменаПриемник, ПредставлениеИсточника) 
	...
	Попытка
		...
		пр_КлиентСервер.Подождать(0.1);
		
		Возврат ИмяФайлаПриемника;	
		
	Исключение
		пр_Ошибка = ОписаниеОшибки();
		ОтправитьСообщениеВТелеграмExchange("РезультатКопированияФайлаОбменаДляПодчиненноУО", , пр_Ошибка, ПредставлениеИсточника);	
		Возврат Неопределено;
	КонецПопытки;  
...
Функция РезультатРаспределенияПоПодчиненнымУО(ТЗ, МассивЗагруженныхФайловОбмена = Неопределено, МассивУО = Неопределено)   Экспорт
	...
	Попытка
		...
		Возврат РезультатРаспределенияДанныхОбменаПоПодчиненнымУОИзРезультатаЗапроса(РезультатЗапроса, НастройкиТекущейБД, ПредставлениеИсточника, МассивЗагруженныхФайловОбмена);
...
Функция ВыполнитьВыгрузкуДанных(НастройкиТекущейБД, НомерВыполнения = 1, СсылкаУО = Неопределено, ЭтоРегламент = Ложь)	Экспорт
	...
	Попытка
		...
		Для каждого ВыборкаУО Из МассивСтруктурУчастниковОбмена  Цикл
			...
			Если ВыборкаУО.ТекущаяБД = Истина Тогда 
...
Функция	ТекстСбораДанныхДляВыгрузки()
	...
	Возврат
		...
		|	оду_БазыУчастникиОбменаСостав.Ссылка.РаспределятьПодчиненнымУО КАК РаспределятьПодчиненнымУО,
		|	оду_БазыУчастникиОбменаСостав.Ссылка.ОтборыОбмена КАК ОтборыОбмена
		|ПОМЕСТИТЬ ВТ_СборДанныхКВыгрузке
		|ИЗ
		|	РегистрСведений.оду_СостояниеОбъектовОбмена КАК оду_СостояниеОбъектовОбмена
		...
		|	оду_СостояниеОбъектовОбмена.ТипXMLОбъектаПриемника КАК ТипXMLОбъектаПриемника,
		|	оду_СостояниеОбъектовОбмена.ИДОбъекта КАК ИДОбъекта,
		|	оду_СостояниеОбъектовОбмена.ПереносТолькоСсылок КАК ПереносТолькоСсылок,
		|	оду_СостояниеОбъектовОбмена.ИДБазыИсточник КАК ИДБазыИсточник,
		|	оду_СостояниеОбъектовОбмена.ИДБазыПриемник КАК ИДБазыПриемник,
		|	оду_СостояниеОбъектовОбмена.РежимОбмена КАК РежимОбмена,
		|	оду_СостояниеОбъектовОбмена.СостояниеОбмена КАК СостояниеОбмена,
		...
		|	оду_РегистрацияОбъектовОбмена.ОбъектОбмена КАК ОбъектОбмена,
		|	оду_СостояниеОбъектовОбмена.НастройкаКонвертацииОбмена КАК НастройкаКонвертацииОбмена,
		|	оду_СостояниеОбъектовОбмена.ДополнительныеСвойстваОбъекта КАК ДополнительныеСвойстваОбъекта,
		|	оду_СостояниеОбъектовОбменаВРежимеЗагрузки.ИДОбъекта ЕСТЬ NULL КАК ОбъектЗагружен,
		//|	оду_РегистрацияОбъектовОбмена.ОбъектОбмена ЕСТЬ НЕ NULL 
		//|		ИЛИ оду_СостояниеОбъектовОбмена.ПриоритетОбмена = 33 КАК Выгружать,
		|		Истина КАК Выгружать,
		|	оду_СостояниеОбъектовОбмена.РаспределятьПодчиненнымУО КАК РаспределятьПодчиненнымУО,
		|	оду_СостояниеОбъектовОбмена.ОтборыОбмена КАК ОтборыОбмена
		|ПОМЕСТИТЬ ВТ_Данные
		|ИЗ
		|	ВТ_СборДанныхКВыгрузке КАК оду_СостояниеОбъектовОбмена
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.оду_СостояниеОбъектовОбмена КАК оду_СостояниеОбъектовОбменаВРежимеЗагрузки
		|		ПО оду_СостояниеОбъектовОбмена.ИДОбъекта = оду_СостояниеОбъектовОбменаВРежимеЗагрузки.ИДОбъекта
		|			И оду_СостояниеОбъектовОбмена.ИДБазыИсточник = оду_СостояниеОбъектовОбменаВРежимеЗагрузки.ИДБазыПриемник
		|			И (оду_СостояниеОбъектовОбменаВРежимеЗагрузки.РежимОбмена = ЗНАЧЕНИЕ(Перечисление.оду_РежимОбмена.ЗагрузкаДанных))
		|			И (оду_СостояниеОбъектовОбменаВРежимеЗагрузки.СостояниеОбмена <> ЗНАЧЕНИЕ(Перечисление.оду_СостоянияОбмена.Загружен))
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.оду_РегистрацияОбъектовОбмена КАК оду_РегистрацияОбъектовОбмена
		...
		|	И оду_СостояниеОбъектовОбмена.НастройкаКонвертацииОбмена <> &КонвертацииОбменаПустая
		|	И оду_СостояниеОбъектовОбмена.НастройкаКонвертацииОбмена.ПометкаУдаления = ЛОЖЬ
		|	И оду_СостояниеОбъектовОбмена.НастройкаКонвертацииОбмена.Используется = ИСТИНА
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		//|ВЫБРАТЬ ПЕРВЫЕ 75
		...
		|	ВТ_Данные.РаспределятьПодчиненнымУО КАК РаспределятьПодчиненнымУО
		|ИЗ
		|	ВТ_Данные КАК ВТ_Данные
		...
		|	И ВТ_Данные.ОбъектЗагружен
		|	И ВТ_Данные.Выгружать
		|
		|УПОРЯДОЧИТЬ ПО
		|	ПриоритетОбмена,
		|	ПриоритетОбъекта,
...
Функция ВыгрузкаДанных(ВыборкаУО, НастройкиТекущейБД) 	Экспорт
	...
	Попытка
		...
		ИмяФайлаОбмена = ИмяФайлаОбмена(НастройкиТекущейБД, ВыборкаУО.ИмяКаталогаПриемника);
...
Функция ТекстЗапросаСбораДанныхОбмена()
	...
	Возврат
		...
		//РаспределятьПодчиненнымУО:
		|	ВЫБОР
		|		КОГДА &ИДБазыИсточник = оду_РегистрацияОбъектовОбмена.ИДБазыПриемник
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.оду_СостоянияОбмена.Выгрузка)
		|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.оду_СостоянияОбмена.ПроверкаДанных)
		|	КОНЕЦ КАК СостояниеОбмена,
...
Функция ТаблицаСостоянийОбмена() Экспорт
	...
	ТЗ.Колонки.Добавить("КонфигурацияКонвертацииОбмена", Новый ОписаниеТипов("ПеречислениеСсылка.оду_КонфигурацияКонвертацииДанных"));
	ТЗ.Колонки.Добавить("ДополнительныеСвойстваОбъекта", ОбщегоНазначения.ОписаниеТипаСтрока(1024));
	ТЗ.Колонки.Добавить("РаспределятьПодчиненнымУО", Новый ОписаниеТипов("Булево"));
...
//TODO:  
//Доработать выгрузки по массиву УО + Распределение (Склад, организация и тд) ПараметрыКонвертации.Вставить("МассивУО", Неопределено);  //МассивУО - которым будут распределяться выгрузка
...
Функция ВыгрузитьОбъектыУОКД2(ВыборкаНастройкиКонвертации, ИмяФайлаОбмена, ВыборкаУО) Экспорт
	...
	Попытка
		...
		Пока ВыборкаНастройкиКонвертации.Следующий() Цикл
			...
			ВыборкаОтборыОбмена = ВыборкаНастройкиКонвертации.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
			Пока ВыборкаОтборыОбмена.Следующий() Цикл  //Отборы Организация, Склады, ВидЦены и др
				
			ПараметрыКонвертации.Вставить("ОтборыОбмена", ВыборкаОтборыОбмена.ОтборыОбмена);
			
			ВыборкаДополнительныеСвойстваОбъекта = ВыборкаОтборыОбмена.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
			
			Пока ВыборкаДополнительныеСвойстваОбъекта.Следующий() Цикл
				
				ПараметрыКонвертации.Вставить("ДополнительныеСвойстваОбъекта", ВыборкаДополнительныеСвойстваОбъекта.ДополнительныеСвойстваОбъекта);
				
				ВыборкаПоТипуXML = ВыборкаДополнительныеСвойстваОбъекта.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
				...
				Пока ВыборкаПоТипуXML.Следующий() Цикл
					...
					Пока ВДЗ.Следующий() Цикл
						...
						Если ЗначениеЗаполнено(ВДЗ.ОбъектОбмена)  Тогда
							ЭлОбмена = ВДЗ.ОбъектОбмена;
						ИначеЕсли  ЗначениеЗаполнено(ВДЗ.ИДОбъекта) И ЗначениеЗаполнено(ВДЗ.ТипXMLОбъектаИсточника) Тогда
							ЭлОбмена = пр_Общий.СсылкаПоТипXML(ВДЗ.ТипXMLОбъектаИсточника, ВДЗ.ИДОбъекта);
						КонецЕсли; 
						
						Если ЭлОбмена = Неопределено Тогда
							Продолжить;
						КонецЕсли; 
						
						МассивСсылок1С.Добавить(ЭлОбмена);
						...
						Если (Сч = КолОбъектовВОбмене ИЛИ КолТекЭл >= КолОбъектовОбмена) //И ИмяТипаXML_До <> Неопределено 
							...
							СтрокаОтветаОбмена = СтрокаОтветаОбменаВыгрузкуДанныхУОДКД2(ТаблицаСостоянийОбмена, Этап, КолЦиклов, ИмяФайлаОбмена, ИмяТипаXML, МассивСсылок1С, 
													СоответвиеПараметровВыгрузки, ОбработкаОбменаДанных, ПараметрыКонвертации, ВДЗ);
							Сч = 0;	
				...
				// Для разных Параметров своя выгрузка, чтобы не грузить по одному чз метод "ВыгрузитьПоПравилу"
			...
			КонецЦикла;  //ВыборкаДополнительныеСвойстваОбъекта
				
			КонецЦикла;  //ВыборкаОтборыОбмена 
		...
		КонецЦикла;  //ВыборкаНастройкиКонвертации 
		
		Возврат РезультатВыгрузки; 
		
	Исключение
		пр_ошибка = ОписаниеОшибки();
		//TODO:  Подумать как реагировать на ошибку. Блокировать всем ?
		//оду_Общий.оду_РезультатЗаписиОшибкиОбработкиУОД(ТаблицаСостоянийОбмена, Объект, пр_ошибка, ОшибкаВПВД);	
		ОтправитьСообщениеВТелеграмExchange("ВыгрузитьОбъектыУО", , пр_ошибка, ВДЗ.ИДБазыПриемник);	
		Возврат Неопределено;
	КонецПопытки; 
...
//TODO:  доработать
Функция СтрокаОтветаОбменаВыгрузкуДанныхУОДКД2(ТаблицаСостоянийОбмена, Этап, КолЦиклов, ИмяФайлаОбмена, ИмяТипаXML,  МассивСсылок1С, СоответвиеПараметровВыгрузки, ОбработкаОбменаДанных, ПараметрыКонвертации, ВДЗ)

	
	ПараметрыКонвертации.Вставить("ИДБазыИсточник", ВДЗ.ИДБазыИсточник);
	ПараметрыКонвертации.Вставить("ИДБазыПриемник", ВДЗ.ИДБазыПриемник);
	ПараметрыКонвертации.Вставить("РаспределятьПодчиненнымУО", ВДЗ.ИДБазыИсточник = ВДЗ.ИДБазыПриемник);
	...
	Для каждого Нстр Из ТаблицаСостоянийОбмена Цикл
		...
		//Нстр.ДополнительныеСвойстваОбъекта = ДополнительныеСвойстваОбъекта;
	КонецЦикла; 
	...
	//СоответвиеПараметровВыгрузки.Вставить(ИмяТипаXML, Новый Структура("оду_МассивСсылок1С, оду_ДополнительныеСвойстваОбъекта", МассивСсылок1С, оду_ДополнительныеСвойстваОбъекта));		
	СоответвиеПараметровВыгрузки.Вставить(ИмяТипаXML, Новый Структура("оду_МассивСсылок1С", МассивСсылок1С));		
	
	Если ПараметрыКонвертации.РаспределятьПодчиненнымУО = Истина Тогда
		СтрокаОтветаОбмена = ВыполнитьВыгрузкуДанныхСРаспределением(ТаблицаСостоянийОбмена, СоответвиеПараметровВыгрузки, ИмяФайлаОбменаДалее, ОбработкаОбменаДанных, ПараметрыКонвертации);
	Иначе
...
Функция ВыполнитьВыгрузкуДанныхУОДКД2(ТаблицаСостоянийОбмена = Неопределено, СоответвиеПараметровВыгрузки = Неопределено, 
	...
	РаспределятьПодчиненнымУО = ПараметрыКонвертации.РаспределятьПодчиненнымУО;
	МассивУО = ПараметрыКонвертации.МассивУО;
	...
	Если ОбработкаОбменаДанных = Неопределено Тогда
		...
		ОбработкаОбменаДанных = ОбработкаОбменаДанныхКД2(ПОД, ПараметрыКонвертации);
	...
	ДоработатьОбработкаОбменаДанных(ОбработкаОбменаДанных, ПараметрыКонвертации);
	...
	СтруктураДанных = ЗаполнитьДанныеОбработкиОбмена(ОбработкаОбменаДанных, ТаблицаСостоянийОбмена, СоответвиеПараметровВыгрузки, ПараметрыКонвертации);	
	
	ЗаполнитьНастройкиОбменаДанных(ОбработкаОбменаДанных, ПараметрыКонвертации);	
	
	Если ПараметрыКонвертации.ИмяПВД = "ВыгрузкаДанныхИзПланаОбмена" Тогда
		//сбрасываются
		//ОбработкаОбменаДанных.Параметры.Вставить("оду_УзелПланаОбмена", ПараметрыКонвертации.оду_УзелПланаОбмена); 	
		ОбработкаОбменаДанных.Данные.Вставить("оду_УзелПланаОбмена", ПараметрыКонвертации.ПланОбмена); 	
	КонецЕсли; 
	...
	//TODO:  доделать 
	Если ТаблицаСостоянийОбмена <> Неопределено И ВыгрузитьЧЗФайл = Ложь Тогда
		ЗаписатьСостоянияПослеВыгрузкиОбъектов(СтруктураДанных.ТаблицаСостоянийОбмена, Результат, РаспределятьПодчиненнымУО, МассивУО, ОбработкаОбменаДанных.мСчетчикВыгруженныхОбъектов);
	КонецЕсли; 
...
// + Доработка по полю 		СостояниеВыгрузки.ДополнительныеСвойстваОбъекта
...
Функция ЗагрузитьДанныеКД2(ВыборкаКонфигурацииКД, НастройкиТекущейБД, ПараметрыКонвертации)
	...
	//Если ВДЗ.Следующий() Тогда //Загрузка осуществляется всегда с актуальным приоритетом, поэтому запрашивает каждый раз
		...
		РезультатЗаписиСостояния = РезультатЗаписиСостоянияПоИмениФайлаПослеЗагрузки(ИмяФайлаОбмена, ВДЗ.ПредставлениеИсточника, ДокументыДляОтложенногоПроведения, НастройкиТекущейБД);
...
//TODO: Доработать	СостояниеВыгрузки.ДополнительныеСвойстваОбъекта,
...
Функция РезультатЗаписиСостоянияПоИмениФайлаПослеЗагрузки(ИмяФайлаОбмена, ПредставлениеИсточника, ДокументыДляОтложенногоПроведения, НастройкиТекущейБД)	Экспорт
	...
	Если ТекстЗапроса = "" Тогда
		ТекстЗапроса = 
			...
			|	оду_СостояниеОбъектовОбмена.РежимОбмена КАК РежимОбмена,  
			|	ВЫБОР
			|		КОГДА оду_СостояниеОбъектовОбмена.РежимОбмена = &РежимОбменаЗапросСсылки
			|			ТОГДА &СостояниеОбменаПроверкаДанных
			|		ИНАЧЕ &СостояниеЗагружен
			|	КОНЕЦ КАК СостояниеОбмена,
			...
			|		КОГДА оду_СостояниеОбъектовОбмена.РежимОбмена = &РежимОбменаЗапросСсылки
			|			ТОГДА """"
			|		ИНАЧЕ оду_СостояниеОбъектовОбмена.ИмяФайлаОбмена
			|	КОНЕЦ КАК ИмяФайлаОбмена,
			|	оду_СостояниеОбъектовОбмена.ПриоритетОбмена КАК ПриоритетОбмена,
			|	оду_СостояниеОбъектовОбмена.ПриоритетОбъекта КАК ПриоритетОбъекта,
			|	оду_СостояниеОбъектовОбмена.ПриоритетДопПризнака КАК ПриоритетДопПризнака,
			|	оду_СостояниеОбъектовОбмена.НомерПопытки КАК НомерПопытки,
			...
			|	оду_СостояниеОбъектовОбмена.ИмяФайлаОбмена = &ИмяФайлаОбмена
			//|ИТОГИ
			//|	МАКСИМУМ(ДополнительныеСвойстваОбъекта)
			//|ПО
			//|	ИмяФайлаОбмена
			|";
	...
	Попытка
		...
		ДопСвойстаОбмена = ДопСвойстаПоИмяФайлаОбмена(ВДЗ);	
		
		//ВыборкаСвойств = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);  
		
		//Пока ВыборкаСвойств.Следующий() Цикл   
		
		РаспределятьПодчиненнымУО	= ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДопСвойстаОбмена,"оду_РаспределятьПодчиненнымУО", Ложь);
		
		Если РаспределятьПодчиненнымУО = Истина Тогда
			
			РезультатРаспределенияПоПодчиненнымУО = РезультатРаспределенияДанныхОбменаПоПодчиненнымУО(НастройкиТекущейБД, ИмяФайлаОбмена, ПредставлениеИсточника);   
			
			Если РезультатРаспределенияПоПодчиненнымУО = Ложь Тогда
				Возврат Ложь;
			КонецЕсли; 
			
		Иначе	 //Проверка на регистрацию
...
Функция ДопСвойстаПоИмяФайлаОбмена(ВДЗ)  

	ВДЗ.Следующий(); 
	
	ДопПараметрыСтрокой = ВДЗ.ДополнительныеСвойстваОбъекта;
	Если ЗначениеЗаполнено(ДопПараметрыСтрокой) Тогда
		ДопСвойста = ЗначениеИзСтрокиВнутр(ДопПараметрыСтрокой);	
		Возврат  ДопСвойста;
	КонецЕсли;  
...
Функция РезультатОчисткиЗаписейПослеЗагрузкиПоИмениФайла(МассивЗагруженныхФайловОбмена, ЭтоПриемник = Ложь, РаспределятьПодчиненнымУО = Ложь)
	...
	Попытка
		...
		Если ВДЗ.Количество() > 0 Тогда
			...
			Если Лев(ВДЗ.ИДОбъекта, 4) = "UOD_" И ЭтоПриемник = Ложь И РаспределятьПодчиненнымУО = Ложь Тогда
...
Функция ЗаполнитьНастройкиОбменаПроизвольногоЗапроса(СтруктураЗапроса)  Экспорт
	...
	Если ЗначениеЗаполнено(СтруктураЗапроса.ПараметрыЗапроса) И ТипЗнч(СтруктураЗапроса.ПараметрыЗапроса) = Тип("Строка") Тогда
	...
	СтруктураЗапроса.Вставить("СсылкаУО", Справочники.оду_БазыУчастникиОбмена.НайтиПоРеквизиту("ИДБазы", ИДБазы));
...
Функция РезультатВыполненияПроизвольногоЗапроса(СтруктураЗапроса, ОбработкаЗапроса = Неопределено, ЭтоЗапрос = Ложь)    Экспорт
	...
	Если СтруктураЗапроса.ПодготовитьДанные = Ложь И ЭтоЗапрос = Ложь И СтруктураЗапроса.СостояниеЗапроса <> оду_ОбщийПовтИсп.СПЗ_ОбработкаОтвета() Тогда
...
Функция СтруктураПараметровВыполненияПроизвольногоЗапроса() Экспорт
	...
	СтруктураЗапроса.Вставить("Пользователь", Строка(пр_НастройкиПовтИсп.ТекущийПользователь()));
...
Функция ЗаписатьДанныеЗапросаВФайл(ДанныхОбмена, УчастникОбмена, ИмяЗапроса, ТипХраненияФайла = "json") Экспорт
	...
	ИмяФайлаЗапроса = ИмяФайлаЗапроса(УчастникОбмена, ИмяЗапроса, ТипХраненияФайла); 
...
Функция ДанныеПроизвольногоЗапросаИзСтруктуры(СтруктураЗапроса)  Экспорт
	...
	Попытка
		Если СтруктураЗапроса.ИмяФайлаЗапроса <> "" Тогда
			...
			ДанныеЗапроса = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ТелоЗапроса, "ДанныеЗапроса", Неопределено);
...
Функция ОтправитьСообщениеВТелеграмExchange(Метод, Объект1С = "", ОшибкаДляТелеграмм = "", БазаУО = "", Версия = "", Данные = Неопределено, ДопИнформация = "")  Экспорт
	...
	ИмяПроцедуры = "оду_Общий.ОтправитьСообщениеВТелеграмExchangeДалее";
	МассивПараметров = Новый Массив;
	МассивПараметров.Добавить(Метод);
	МассивПараметров.Добавить(Объект1С);
	МассивПараметров.Добавить(ОшибкаДляТелеграмм);
	МассивПараметров.Добавить(БазаУО);
	МассивПараметров.Добавить(Версия);
	МассивПараметров.Добавить(Данные);
	МассивПараметров.Добавить(ДопИнформация);
	
	пр_Общий.ЗапуститьФоновоеВыполнение(ИмяПроцедуры, МассивПараметров, , "оду_ОтправитьСообщениеВТелеграмExchangeДалее"); 
	
КонецФункции

//БазаИсточник, Метод, Объект1С = "", ОшибкаДляТелеграмм = "", БазаПриемник = "", Версия = "", Данные = Неопределено, ДопИнформация = ""
Функция ОтправитьСообщениеВТелеграмExchangeДалее(Метод, Объект1С = "", ОшибкаДляТелеграмм = "", БазаУО = "", Версия = "", Данные = Неопределено, ДопИнформация = "")  Экспорт
